...
class UI {
    ...
    async showMainMenu() {
        while (true) {
            try {
                ...
                const urlsCount = this.optionsManager.loadUrls().length;
   ...
   */
    ...
    async handleAnalyzeFile() {
        ...
        if (urlsCount === 0) {
            console.log(boxManager.createWarningBox('⚠️ Nenhuma URL encontrada no arquivo links.txt.'));
            console.log(colors.text('📝 Adicione URLs através do menu "📝 URLs" primeiro.'));
            await this.delay(2000);
            return;
        }
    ...
    async handleEditTags() {
        try {
            ...
            this.optionsManager.saveTags(updatedTags);
            ...
            logger.error('Erro ao editar tags:', error);
            console.log(boxManager.createErrorBox('❌ Erro ao salvar tags.'));
        }
    }
    ...
    async handleAddUrl() {
        ...
        this.optionsManager.addUrl(url.trim());
    ...
    async handleEditUrls() {
        try {
            ...
            logger.error('Erro ao editar URLs:', error);
            console.log(boxManager.createErrorBox('❌ Erro ao salvar URLs.'));
        }
    }
    ...
    async pressToContinue() {
        ...
        console.log(colors.muted('⏎ Pressione Enter para voltar ao menu principal...'));

        try {
            // Usar readline em vez de inquirer para evitar conflitos
